...
export default async function handler(req: any, res: any) {
    ...
    switch (method) {
        case 'GET':
            try {
                if (id === 'pnl') {
                    ...
                    // Giá vốn = cost_price của các orders đã paid (có accounts)
                    let cogs = 0;
                    for (const o of paidOrders) {
                        cogs += (o.cost_price || 0) * (o.quantity || 1);
                    }
        ...
        case 'POST':
            try {
                const { supplier_id, category, amount, description, recurring_config, linked_order_id, linked_product_id } = req.body;
