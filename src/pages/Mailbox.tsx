...
import {
    ...
    FiInbox, FiEdit3, FiSettings, FiPlus, FiAlertCircle
} from 'react-icons/fi';
...
const Mailbox: React.FC = () => {
    ...
    return (<div>
        ...
        {imapEmail && (<div style={{ background: '#F0F9F1', borderRadius: 12, padding: '0.6rem 1rem', marginBottom: '1rem', border: '1px solid #C2E7C6', display: 'flex', alignItems: 'center', gap: 8 }}>
            <FiCheckCircle style={{ color: '#2E7D32', fontSize: 16 }} />
            <span style={{ fontSize: 13, color: '#2E7D32', fontWeight: 600 }}>✅ Đã kết nối: {imapEmail}</span>
            <button onClick={() => setShowSettings(true)} style={{ marginLeft: 'auto', border: 'none', background: 'transparent', color: '#0071E3', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>Cấu hình lại</button>
        </div>)}
        ...
        <div style={{ display: 'flex', gap: '1rem', height: 'calc(100vh - 200px)', overflow: 'hidden' }}>
            <div style={{ width: 200, minWidth: 170, background: '#F8F9FC', borderRadius: 14, padding: '1rem', border: '1px solid #EEE', display: 'flex', flexDirection: 'column', gap: '0.3rem' }}>
                {tabFilters.map(t => (<button key={t.key} onClick={() => { setActiveTab(t.key); setSelected(null); }} style={{ border: 'none', background: activeTab === t.key ? '#0071E3' : 'transparent', color: activeTab === t.key ? '#FFF' : '#1D1D1F', padding: '8px 12px', borderRadius: 8, fontWeight: 600, fontSize: 13, cursor: 'pointer', textAlign: 'left', display: 'flex', alignItems: 'center', gap: 6 }}>{t.icon} {t.label}</button>))}
                <div style={{ flex: 1 }} />
                <button onClick={() => setShowCompose(true)} style={{ border: 'none', background: '#34C759', color: '#FFF', padding: '10px', borderRadius: 8, fontWeight: 600, fontSize: 13, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, marginBottom: 6 }}><FiPlus /> Soạn mail</button>
                <button onClick={syncEmails} disabled={syncing} style={{ border: 'none', background: imapEmail ? '#0071E3' : '#8E8E93', color: '#FFF', padding: '10px', borderRadius: 8, fontWeight: 600, fontSize: 13, cursor: imapEmail ? 'pointer' : 'not-allowed', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}><FiRefreshCw /> {syncing ? 'Đang đồng bộ...' : 'Đồng bộ Gmail'}</button>
                {imapEmail && (<button onClick={() => setShowSettings(true)} style={{ border: 'none', background: 'transparent', color: '#8E8E93', padding: '6px', borderRadius: 8, fontSize: 11, cursor: 'pointer' }}><FiSettings style={{ marginRight: 4, verticalAlign: 'middle' }} /> Cài đặt IMAP</button>)}
            </div>
                    ...
                    <table className="styled-table">
                        ...
                        <tbody>
                            {loading ? <tr><td colSpan={4} style={{ textAlign: 'center', padding: '2rem' }}>Đang tải...</td></tr> :
                                messages.length > 0 ? messages.map(m => (<tr key={m._id} onClick={() => { setSelected(m); setReplyText(m.ai_draft || ''); }} style={{ cursor: 'pointer', background: selected?._id === m._id ? '#EBF5FF' : m.is_read ? '#FFF' : '#FFFDF0', fontWeight: m.is_read ? 400 : 600 }}>
                                    <td className="nowrap" style={{ fontSize: 13 }}>{m.from_name || m.from}{m.linked_customer_id && <span style={{ marginLeft: 6, fontSize: 10, color: '#30D158' }}><FiUser style={{ verticalAlign: 'middle' }} /></span>}</td>
                                ...
                                </tr>)) : <tr><td colSpan={4} style={{ textAlign: 'center', padding: '2rem', color: '#8E8E93' }}>{imapEmail ? 'Chưa có mail nào. Nhấn "Đồng bộ Gmail" để tải thư về.' : 'Vui lòng cấu hình IMAP trước.'}</td></tr>}
                        </tbody>
                    </table>
                </div>
            </div>
                ...
                {selected ? (<div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
                    <div style={{ padding: '1rem', borderBottom: '1px solid #EEE', background: '#FAFAFC' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                            <div><h3 style={{ margin: 0, fontSize: '1rem' }}>{selected.subject}</h3><p style={{ margin: '4px 0 0', fontSize: 12, color: '#8E8E93' }}>{(selected.from_name || selected.from) + ' <' + selected.from + '>'}{selected.linked_customer_id && <Link to={'/customers/' + selected.linked_customer_id._id} style={{ marginLeft: 8, color: '#0071E3', fontWeight: 600 }}><FiUser style={{ verticalAlign: 'middle' }} /> {selected.linked_customer_id.name}</Link>}</p></div>
                            <select value={selected.status} onChange={e => updateStatus(selected._id, e.target.value)} style={{ padding: '4px 8px', borderRadius: 6, border: '1px solid #D2D2D7', fontSize: 11 }}><option value="new">Mới</option><option value="pending">Đang xử lý</option><option value="resolved">Đã xong</option></select>
                        </div>
                        <div style={{ display: 'flex', gap: 4, marginTop: 6 }}>{selected.tags?.map(t => <span key={t} style={{ padding: '2px 8px', borderRadius: 99, fontSize: 10, fontWeight: 700, background: (tagColor[t] || '#888') + '15', color: tagColor[t] || '#888' }}>{t}</span>)}</div>
                    </div>
                    <div style={{ flex: 1, overflowY: 'auto', padding: '1rem', fontSize: 13, lineHeight: 1.6 }} dangerouslySetInnerHTML={{ __html: selected.body_html || (selected.body_text || '').replace(/\n/g, '<br>') }} />
                    <div style={{ padding: '1rem', borderTop: '1px solid #EEE', background: '#FAFAFC' }}>
                        {selected.ai_draft && !replyText && (<div style={{ marginBottom: 8, background: '#EBF5FF', borderRadius: 8, padding: 8, fontSize: 12, color: '#0071E3', display: 'flex', alignItems: 'center', gap: 6 }}><span>🤖 AI đã soạn sẵn câu trả lời</span><button onClick={useAiDraft} style={{ marginLeft: 'auto', border: 'none', background: '#0071E3', color: '#FFF', padding: '4px 10px', borderRadius: 6, fontSize: 11, cursor: 'pointer', fontWeight: 600 }}>Dùng bản nháp</button></div>)}
                        <textarea value={replyText} onChange={e => setReplyText(e.target.value)} rows={4} placeholder="Soạn câu trả lời..." style={{ width: '100%', padding: '0.6rem', borderRadius: 8, border: '1px solid #D2D2D7', fontSize: 13, resize: 'vertical' }} />
                        <div style={{ display: 'flex', gap: 8, marginTop: 8 }}><button onClick={handleSendReply} style={{ border: 'none', background: '#0071E3', color: '#FFF', padding: '8px 20px', borderRadius: 8, fontWeight: 600, fontSize: 13, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5 }}><FiSend /> Gửi trả lời</button><button onClick={() => { setReplyText('Cảm ơn bạn đã liên hệ. Chúng tôi sẽ phản hồi sớm nhất.\n\nTrân trọng,\nBeegadget.net'); }} style={{ border: 'none', background: '#F5F5F7', color: '#1D1D1F', padding: '8px 12px', borderRadius: 8, fontSize: 12, cursor: 'pointer' }}><FiEdit3 style={{ marginRight: 4, verticalAlign: 'middle' }} /> Mẫu</button></div>
                    </div>
